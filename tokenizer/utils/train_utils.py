"""Unified training utilities for all stages.
...
def create_optimizer(config, logger, model, loss_module,
    ...
    """Creates optimizer for model and discrminator.
    
    Supports two modes based on config.optimizer.params.use_encoder_lr_scale:
    - False (default, for stage1): All parameters use the same learning rate
    - True (for stage2): Encoder parameters use scaled learning rate
    """
    ...
    # Check if we should use different LR for encoder
    use_encoder_lr_scale = optimizer_config.get("use_encoder_lr_scale", False)
    encoder_lr_scale = optimizer_config.get("encoder_lr_scale", 0.1)
    ...
    if use_encoder_lr_scale:
        # Stage2 style: encoder/mlp1 uses scaled LR, others use full LR
        exclude_encoder = (lambda n, p: ('encoder' in n or 'mlp1' in n) and 
                          (p.ndim < 2 or "ln" in n or "bias" in n or 'latent_tokens' in n 
                           or 'mask_token' in n or 'embedding' in n or 'norm' in n or 'gamma' in n or 'embed' in n))
        include_encoder = lambda n, p: not exclude_encoder(n, p) and ('encoder' in n or 'mlp1' in n) 
        exclude_other = (lambda n, p: ('encoder' not in n and 'mlp1' not in n) and 
                        (p.ndim < 2 or "ln" in n or "bias" in n or 'latent_tokens' in n 
                         or 'mask_token' in n or 'embedding' in n or 'norm' in n or 'gamma' in n or 'embed' in n))
        ...
        optimizer = optimizer_cls(
            [
                {"params": gain_or_bias_params_encoder, "weight_decay": 0., 'lr': learning_rate * encoder_lr_scale},
                {"params": rest_params_encoder, "weight_decay": optimizer_config.weight_decay, 'lr': learning_rate * encoder_lr_scale},
        ...
        logger.info(f"Using encoder LR scale: {encoder_lr_scale}")
    else:
        # Stage1 style: All parameters use same LR
    ...
    if need_discrminator:
        print("Setting up discriminator optimizer")
        ...
        print("No discriminator optimizer")
...
def create_lr_scheduler(config, logger, accelerator, optimizer, discriminator_optimizer=None):
    """Creates learning rate scheduler for model and discrminator."""
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.log_every == 0:
                ...
                # Build log message based on available losses
                log_msg = (
                ...
                if accelerator.unwrap_model(loss_module).should_discriminator_be_trained(global_step):
                    logits_real = discriminator_logs.get('train/logits_real', 0.0)
                    logits_fake = discriminator_logs.get('train/logits_fake', 0.0)
                    log_msg += f"Logits Real: {logits_real:0.4f} Logits Fake: {logits_fake:0.4f} "
                ...
                if accelerator.is_main_process:
                    wandb.log(logs, step=global_step + 1)
...
def log_grad_norm(model, accelerator, global_step):
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if accelerator.is_main_process:
                wandb.log({"grad_norm/" + name: grad_norm}, step=global_step)
