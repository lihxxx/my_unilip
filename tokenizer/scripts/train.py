"""Unified training script for all stages.
...
def main():
    ...
    accelerator = Accelerator(
        ...
        split_batches=False,
        kwargs_handlers=[
            DistributedDataParallelKwargs(find_unused_parameters=config.training.get("ddp_find_unused_parameters", False))
        ]
    )
    ...
    stage_name = "Stage 2 (with Distillation)" if use_distill else "Stage 1 (Decoder Training)"
    logger.info(f"Training mode: {stage_name}")
    ...
    # Start training.
    logger.info(f"***** Running {stage_name} Training *****")
