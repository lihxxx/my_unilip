...
关键参数:
    ...
    --image_size 512/448             # Stage1用512，Stage2/3用448
    --use_vae_image_norm True/False  # Stage1需要True
    --fix_dit True/False             # Stage1用True，Stage2/3用False
...
class DataArguments:
    ...
    gen_image_folder: Optional[str] = field(default=None, metadata={"help": "生成数据路径，多个路径用逗号分隔"})
    edit_image_folder: Optional[str] = field(default=None, metadata={"help": "编辑数据路径，多个路径用逗号分隔"})
    ...
    image_size: int = field(default=448, metadata={"help": "图像大小"})
    use_vae_image_norm: bool = field(default=False, metadata={"help": "使用VAE图像标准化（mean=0.5, std=0.5）"})
...
class LazySupervisedMixDataset(Dataset):
    ...
    def load_gen(self, gen_path):
        """加载生成数据，支持多路径（逗号分隔）"""
        # 支持多个路径，用逗号分隔
        paths = [p.strip() for p in gen_path.split(',')]
        ...
        if not data_files:
            raise ValueError(f"No tar files found in paths: {paths}")
        
        rank0_print(f"Loading gen data from {len(paths)} path(s), found {len(data_files)} tar files")
        ...
        train_dataset = train_dataset.remove_columns([col for col in train_dataset.column_names if col not in 
    ...
    def load_edit(self, edit_path):
        """加载编辑数据，支持多路径（逗号分隔）"""
        # 支持多个路径，用逗号分隔
        paths = [p.strip() for p in edit_path.split(',')]
        ...
        rank0_print(f"Loading edit data from {len(paths)} path(s), found {len(data_files)} tar files")
...
def train(attn_implementation=None):
    ...
    print(f"use_vae_model: {model_args.use_vae_model}")
    print(f"image_size: {data_args.image_size}")
    print(f"use_vae_image_norm: {data_args.use_vae_image_norm}")
    ...
    # 根据use_vae_model选择模型类
    if model_args.use_vae_model:
    ...
    # 设置图像大小
    data_args.image_processor.size['height'] = data_args.image_size
    data_args.image_processor.size['width'] = data_args.image_size
    
    # 使用VAE图像标准化（mean=0.5, std=0.5）
    if data_args.use_vae_image_norm:
